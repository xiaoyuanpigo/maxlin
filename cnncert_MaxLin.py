...
def pool_linear_bounds(LB, UB, pad, stride, pool_size):
    """
    These are the linear bounds--MaxLin
    """
    p_hl, p_hr, p_wl, p_wr = pad
    s_h, s_w = stride
    ...
    for x in range(alpha_u.shape[2]):
        for y in range(alpha_u.shape[3]):
            for r in range(alpha_u.shape[4]):
                ...
                idx = np.where(cropped_UB>=(-np.inf))
                ...
                for i in range(len(idx[0])):
                    ...
                    m_s[i] = (cropped_LB[idx[0][i],idx[1][i]] +cropped_UB[idx[0][i],idx[1][i]])/2
                ...
                elif u_s[iu]>l_s[il] and  u_s[ju]<=l_s[il] and l_s[iu]>=l_s[il]:
                    weights[il] = 1
                    intercept = l_s[il] 

                else:
                    weights[iu] = (u_s[iu]- u_s[ju]) / (u_s[iu] - l_s[iu])
                    intercept = u_s[ju]
